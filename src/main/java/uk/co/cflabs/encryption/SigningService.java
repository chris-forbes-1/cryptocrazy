...
public class SigningService
{
	...
	public EncryptedResponse signMessage(EncryptedResponse response)
	{
		logger.info("Signing message: [{}]", response.getEncryptedText());
		response.setSignature(signMessage(response.getEncryptedText()));
		response.setSigningKeyPair(keyPair);
		logger.info("Signed message:[{}]", response.getSignature());
		return response;
	}
	...
	public boolean verify(EncryptedResponse response)
	{
